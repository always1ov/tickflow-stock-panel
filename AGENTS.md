...
16. **撤下或隐藏一个功能，同一个提交里登记进 [`docs/hidden-features.md`](docs/hidden-features.md)。**
    ...
    - 后端还留着、只是页面没入口 → 写进「一、隐藏」，并在
      [`test_hidden_features_registry.py`](backend/tests/test_hidden_features_registry.py) 的
      `STILL_THERE` 加一个锚点，让「还在」一直是真的。
    - 代码删了 → 写进「二、撤下」，带上提交号（找回用 `git show <提交号>^:<路径>`）。
    - 数据文件留在磁盘上 → 必须仍登记在数据体检 `data_doctor.STORES` 里，`note` 写明「功能已撤」。
    - 只收**功能级**的撤除；改一行文案、挪一个按钮不算。
    - 用户问「以前那个 XX 还能用吗」，先查这份登记簿，再查 `FORK_NOTES`。
