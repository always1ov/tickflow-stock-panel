...
输出: 结构化价位点,按组返回(组名见下面的 LEVEL_TYPES),供:
  - 图表 markLine 渲染(压力位 / 支撑位 / 成交密集区 / 枢轴点 / 前高前低 …)
  - AI 个股分析提示词(价位上下文)

[R403] 这里原来写的是「4 类」, 而 `compute_levels` 早就算 11 类了 —— 数字写死在
注释里必然过期, 所以不再写数, 组名以 LEVEL_TYPES 为准。
...
LEVEL_TYPES = {
    ...
    "gap": "缺口位",          # 未回补跳空缺口
    "fib": "斐波那契Ⅰ型",    # 回撤位 0.236~0.786(近 120 日单一波段)
    "round": "整数关口",      # 心理整数位
    # [fork 增强] 六态关键点(利弗莫尔上/下关键点)。不在 compute_levels 内计算 ——
    # 需要每票各自的阈值配置,由 API 层(stock_analysis.get_levels)注入。
    "livermore": "六态关键点",
    # [R405 · fork 增强] 斐波那契二型(帝纳波利点位)。同样不在 compute_levels 内算 ——
    # 它除了横线还有色带/标记/均线, 塞不进"一组横线"的结构, 由 API 层
    # (stock_analysis.get_levels)注入。与上面的「一型」是两套算法, 见那一行的说明。
    "fib2": "斐波那契Ⅱ型",
    # [R403] 「持仓止盈」这一组**从图上撤了**(用户: 「持仓止盈可以删除掉了」)。
    #
    # **撤的只是这张图上的一组线, 出场线系统本身一个字没动**:
    # `services/position_exit.py` 原样在跑, 决策台那一列、盘中 `exit_*`/`exitlife_*`
    # 监控推送全部照旧([R435] 原来这里还列着「AI 信号里的持仓上下文」, AI 信号
    # 已整套停用)。要把线加回图上, 恢复这一行 + API 层那段注入即可。
}
