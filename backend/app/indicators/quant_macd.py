...
## 别照通达信「动态翻译」的字面去改 OBV1 / OBV3

通达信自带的动态翻译是**按模板拼字符串**的: `SUM(X,0)` → 「X的历史累和」,
`EMA(X,3)` → 「X的3日指数移动平均」, `IF(c,A,B)` → 「如果c,返回A,否则返回B」。
X 本身是一个 IF 时, 拼出来就成了:

    OBV1: 「如果收盘价=1日前的收盘价,返回0,否则返回VA的历史累和」
    OBV3: 「如果OBV2>0,返回OBV2,否则返回0的3日指数移动平均」

**读起来像是累和 / 平均只套在最后那个值上, 其实套的是整个 IF** —— 原文是
`SUM(IF(...),0)` 与 `EMA(IF(...),3)`, 括号说了算, 通达信执行的是原文不是翻译。
按字面那样改, OBV1 会在每个平盘日掉回 0、OBV3 会失去平滑, 两种读法测试都会红
(R415 对这两种读法各做过一次变异, 均被杀掉)。
