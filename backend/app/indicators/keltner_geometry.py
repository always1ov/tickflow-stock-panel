...
# ================================================================
# [R223] 六态与阶段对不上的时候, 得说出来

# 阶段落在哪一侧。`coiling` / `unclear` 是真中性 —— 它们本来就不表态。
_PHASE_SIDE = {
    PH_ADVANCING: "up", PH_LAUNCHING: "up", PH_STALLING: "up",
    PH_DECLINING: "down",
}


def trend_phase_gap(state: str | None, ph: dict | None,
                    geo: dict | None = None) -> dict | None:
    """六态与通道阶段是不是在说相反的话。是就返回一句解释, 否则 None。

    ## 用户撞见的是什么

    「好多不对的……不然图形对不上结论啊」。截图里并排着这样几行:

        自然回升 10天(多头)  +  下跌中
        下跌趋势 6天(空头)   +  涨势转弱

    这不是某一边算错了 —— **两边量的根本不是同一个东西**:

        六态   看的是**价格的高低点**(利弗莫尔关键点), 价格一转向它就转
        阶段   看的是**三条均线的中枢**, 而均线是滞后算子

    所以在**转折那一段**它们必然对不上, 而且恰恰是这一段最值得知道: 价格
    已经转了、均线还没跟上(或者反过来)。R215 修过其中一种(价格跌穿三条线
    还叫「涨势转弱」), 但那只是这件事的一个特例 —— 一般情形直到这里才处理。

    ## 为什么是「说出来」而不是「改一个去迁就另一个」

    改哪个都是错的: 六态是作者的判定(禁止动), 阶段是从均线几何推出来的
    (它没算错)。**两个都对, 只是看的东西不同。** 与 `stock_playbook._conflicts`
    同一条纪律 —— 打架的时候不替用户选边, 但必须让他看见在打架。

    只认**真正相反**的: 横盘中 / 看不出这类不表态的一律不算, 否则一半的行
    都会挂个标记, 那这个标记三天后就没人看了(R205 立的规矩)。
    """
    if not ph or not state:
        return None
    side = _PHASE_SIDE.get(ph.get("code"))
    if ph.get("code") == PH_OVEREXTENDED:
        # 走过头分两个方向, 看间距的符号
        sp = (geo or {}).get("spread")
        side = None if sp is None else ("up" if sp > 0 else "down")
    if side is None:
        return None
    bull = state in _BULL_STATES
    if bull and side == "up":
        return None
    if not bull and side == "down":
        return None
    cn = (ph.get("cn") or "")
    if bull:      # 六态多头, 阶段却在跌势侧
        return {
            "cn": "六态与通道不一致",
            "why": f"六态看价格的高低点, 已经转到多头侧; 而「{cn}」是按三条均线的"
                   f"中枢读的, 均线还在下面。**价格先转、均线后转**, 转折那几天"
                   f"本来就会这样 —— 别当成其中一个算错了。要确认的是: 价格这次"
                   f"转向能不能站住到把均线也带上来。",
        }
    return {
        "cn": "六态与通道不一致",
        "why": f"六态看价格的高低点, 已经转到空头侧; 而「{cn}」是按三条均线的"
               f"中枢读的, 均线还挂在上面 —— **均线是滞后的**, 涨完那一段还留在"
               f"它身上。这种时候以价格为准: 均线好看不代表这一段还没走完。",
    }
