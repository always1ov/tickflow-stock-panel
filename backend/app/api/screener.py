...
def _cached_with_realtime(request: Request) -> dict:
    ...
    # 自选实时(watchlist 档)下不叠加实时结果: 该档每轮只拉 ~5 只自选, 监控引擎算出的
    # 策略结果只覆盖自选, 若叠加会把被监控策略的全市场盘后结果替换成「只剩自选」。
    # 该档全市场选股本就是盘后(无全市场实时权限), 因此保持盘后缓存, 不做实时覆盖。
    try:
        from app.services.quote_service import QuoteService
        if QuoteService.realtime_mode() == "watchlist":
            return cached
    except Exception:  # noqa: BLE001
        pass
...
def limit_ladder(
    ...
    # ── 部分数据日防线 ──
    # 免费档盘后、辅助源当日数据未发布时, 当日分区可能只有自选实时快照写入的
    # 少数股票 —— 用它算梯队会得出「涨停0、只有几只断/炸板」的假象。
    # 与前一交易日全市场规模对比, 悬殊(<50%)即判定不完整:
    #   默认日期 → 自动回退上一完整交易日并标注; 显式指定日期 → 保留但标注。
    partial_from: str | None = None
    partial_count: int | None = None
    prior_d, prior_n = svc.prior_partition_info(as_of)
    if prior_n > 0 and df.height < prior_n * 0.5:
        partial_count = df.height
        if not explicit_as_of and prior_d is not None:
            fb = svc._load_enriched_for_date(prior_d)
            if not fb.is_empty():
                partial_from = str(as_of)
                as_of = prior_d
                df = fb
                partial_count = None
        if partial_count is not None:
            # 显式指定的部分数据日: 保留该日数据, 仅标注
            partial_from = str(as_of)
    ...
    return {
        "as_of": str(as_of),
        # 部分数据日标注: partial_from=不完整的原始日期, partial_count=该日仅有的股票数(None=已回退)
...
# ================================================================
# 连板梯队 · AI 战法清单 (fork #23)
# ================================================================

class LadderAiRequest(BaseModel):
    """连板梯队快照 → AI 按打板战法输出候选清单(支持多轮追问 + 历史报告续问)。"""
    date: str = ""
    stats: dict = Field(default_factory=dict)
    tiers: list = Field(..., min_length=1, max_length=12)
    messages: list = Field(default_factory=list, max_length=16)
    report_id: str = ""   # 传入已存报告 id: 追问时复用该报告存档的快照上下文
...
_LADDER_AI_SYSTEM = (
    "你是 A 股打板复盘助手, 精通短线情绪周期与连板战法。用户给出某交易日连板梯队快照"
    "(boards=连板数; status: limit_up=涨停收盘/broken=炸板/failed=断板; sealed_status: "
    "real=真封单/fake=假涨停/pending=待确认; is_one_word=一字板; concepts=所属题材)。\n"
    "运用以下方法论(战法知识库):\n"
    "· 情绪周期: 用涨停家数/炸板率(炸板÷(涨停+炸板))/最高板高度判断 冰点/回暖/发酵/高潮/退潮;"
    "退潮期一切接力战法胜率骤降, 应明确提示控制仓位或观望。\n"
    "· 龙头战法: 最高板且题材有梯队跟风者为总龙头; 龙头看辨识度与题材地位, 不轻易切换;"
    "高位一字板无法买入, 只跟踪不列为可操作。\n"
    "· 二进三/接力: 二板是分歧转一致的关键位; 优先 真封单+非一字+题材内有首板跟风 的二板;"
    "缩量加速优于放量滞涨; 孤板(题材无跟风)降级。\n"
    "· 首板卡位: 涨停家数放大时, 强题材首板(真封单/放量换手)可关注次日溢价。\n"
    "· 断板反包/炸板回封: 昨日断板或今日炸板的人气股若明日反包为强承接信号, 列入观察而非买入。\n"
    "首轮输出(纯文本, ≤500字):\n"
    "1) 情绪定性一句(引用涨停/炸板/最高板数字, 若有市场环境/主线排行数据必须一并引用), 末尾标「置信 NN%」;\n"
    "2) 总龙头判定 + 一句理由(无明确龙头就直说), 末尾标「置信 NN%」;\n"
    "3) 候选清单分组:【接力关注】【低吸/卡位关注】【反包观察】【回避】——每条: 名称(代码) · N板 · "
    "战法标签 · 一句理由 · 「置信 NN%」; 每组最多 4 条, 宁缺毋滥; 置信度要拉开区分度, 不许全部相同;\n"
    "4) 一句风险提示。之后若用户追问, 基于同一快照直接回答(≤250字)。"
    "打板为高风险行为, 输出仅为复盘参考, 不是投资建议。"
)

_LADDER_STOCK_KEEP = ("symbol", "name", "boards", "status", "change_pct",
                      "sealed_status", "is_one_word", "concepts")


def _ladder_ai_enrichment(request: Request) -> dict:
    """服务端富化: 市场环境(regime 最新) + 概念/行业领涨排行 + 市场宽度。

    各源尽力而为, 失败即缺项(不阻断主流程)。这些信号帮 AI 做两件事:
    情绪周期定性有客观依据(regime 状态/评分), 题材是否"当前主线"有排行佐证。
    """
    out: dict = {}
    try:
        from app.services import regime_builder
        df = regime_builder.load_regime_history(request.app.state.repo.store.data_dir)
        if not df.is_empty():
        ...
        logger.debug("ladder-ai regime 富化失败(跳过): %s", e)
    try:
        from app.services.market_overview_builder import build_market_overview
        ov = build_market_overview(
            repo=request.app.state.repo,
        ...
        def _rank(items):
            return [
                {k: it.get(k) for k in ("name", "change_pct", "up", "down") if k in it}
                for it in (items or [])[:5]
            ]
        out["市场宽度"] = ov.get("breadth")
        out["情绪分"] = ov.get("emotion")
        out["概念领涨"] = _rank((ov.get("concept_rank") or {}).get("leading"))
        out["概念领跌"] = _rank((ov.get("concept_rank") or {}).get("lagging"))
        out["行业领涨"] = _rank((ov.get("industry_rank") or {}).get("leading"))
    except Exception as e:  # noqa: BLE001
        logger.debug("ladder-ai overview 富化失败(跳过): %s", e)
    return out


@router.post("/ladder-ai")
async def ladder_ai_review(req: LadderAiRequest, request: Request):
    """把连板梯队快照交给 AI, 按打板战法(龙头/二进三/反包等)输出分组候选清单。

    - 战法知识固化在系统提示词(稳定可控, 不实时抓网页); 快照字段白名单过滤
    - 服务端自动富化: 市场环境(regime) + 概念/行业领涨排行 + 市场宽度
    - 首轮结果自动存档(ladder_ai_reports.json, 保留 30 份), 供历史回看
    - messages 支持追问; 传 report_id 可对历史报告续问(复用其存档快照)
    """
    ...
    if req.report_id and req.messages:
        stored = next((r for r in _ladder_reports.list_reports() if r.get("id") == req.report_id), None)
        if stored and stored.get("context"):
            context = str(stored["context"])

    if context is None:
        stats_keep = ("limit_up", "limit_down", "broken", "failed", "max_boards")
        tiers_clean = []
        for t in req.tiers[:12]:
            if not isinstance(t, dict):
                continue
            stocks = [
                {k: s.get(k) for k in _LADDER_STOCK_KEEP}
                for s in (t.get("stocks") or [])[:25]
                if isinstance(s, dict)
            ]
            tiers_clean.append({"boards": t.get("boards"), "count": t.get("count"), "stocks": stocks})
        context = json.dumps({
            "日期": req.date,
            "情绪统计": {k: req.stats.get(k) for k in stats_keep},
            **_ladder_ai_enrichment(request),
            "梯队": tiers_clean,
        }, ensure_ascii=False)
    ...
    try:
        text = await generate_ai_text(
            [
                {"role": "system", "content": _LADDER_AI_SYSTEM},
    ...
    if not history:  # 首轮(非追问) → 自动存档, 历史可回看
        try:
            saved = _ladder_reports.save_report({
                "date": req.date,
                "context": context,
                "text": out_text,
            })
            ...
            logger.warning("战法清单存档失败(不影响返回): %s", e)
    return {"text": out_text, "report_id": report_id}


@router.get("/ladder-ai/reports")
def ladder_ai_reports():
    """历史战法清单(按时间降序, 不含快照上下文)。"""
    return {"reports": [
        {"id": r.get("id"), "date": r.get("date"), "created_at": r.get("created_at"), "text": r.get("text")}
        for r in _ladder_reports.list_reports()
    ]}


@router.delete("/ladder-ai/reports/{report_id}")
def ladder_ai_delete_report(report_id: str):
    """删除一份历史战法清单。"""
    return {"ok": _ladder_reports.delete_report(report_id)}
