...
class HealthInterpretRequest(BaseModel):
    """策略体检对比表 → AI 解读(支持多轮追问)。rows 为前端聚合后的每策略指标;
    messages 为既有对话(assistant=此前解读, user=追问), 空 = 首轮解读。"""
    rows: list[dict] = Field(..., min_length=1, max_length=30)
    window_label: str = ""
    messages: list[dict] = Field(default_factory=list, max_length=16)


@router.post("/strategy/health-interpret")
async def strategy_health_interpret(req: HealthInterpretRequest):
    """把「策略体检」的胜率对比表交给 AI, 输出"留谁/调谁/怎么调"的一段建议。

    只读输入指标, 不访问行情; 未配置 AI 时报 400 由前端 toast 提示。
    """
    from app.services.ai_provider import ai_configured, generate_ai_text

    if not ai_configured():
        raise HTTPException(status_code=400, detail="未配置 AI, 无法生成解读")

    # 只保留已知指标字段, 防御性过滤 (前端聚合数据, 不直接透传任意 dict 给提示词)
    keep = ("name", "win_rate", "profit_factor", "total_return",
            "max_drawdown", "n_trades", "avg_pnl")
    rows = [{k: r.get(k) for k in keep} for r in req.rows]

    system = (
        "你是量化策略评估助手。用户给出多个选股策略同窗口回测的指标对比"
        "(win_rate=胜率, profit_factor=盈亏比, total_return=区间收益, "
        "max_drawdown=最大回撤, n_trades=交易笔数, avg_pnl=平均单笔收益, 比例均为小数)。"
        "首轮请输出中文建议, 要求:\n"
        "1) 按「值得保留 / 建议调整 / 建议放弃」分组点评, 每条一句话并引用具体数字;\n"
        "2) 胜率与盈亏比要结合看(高胜率低盈亏比=赢小亏大要点破);\n"
        "3) n_trades<30 的策略必须明确提示样本不足、结论不可靠;\n"
        "4) 结尾一句话总结。纯文本输出(可用序号), 全文≤300字。\n"
        "之后若用户追问, 基于同一张表直接回答问题, 引用具体数字, ≤250字。均不是投资建议。"
    )
    context = (
        f"回测窗口: {req.window_label or '未注明'}\n"
        f"指标对比(JSON): {json.dumps(rows, ensure_ascii=False)}"
    )
    # 追问轮: 附上既有对话(role 白名单 + 内容截断), 首轮 messages 为空
    history = []
...
class DiagnoseRequest(BaseModel):
    """策略诊断: 配置 + 回测结果证据 → AI 归析死因并给具体改法。前端聚合后传入。"""
    name: str = ""
    window_label: str = ""
    scope_label: str = ""
    config: dict = Field(default_factory=dict)
    stats: dict = Field(default_factory=dict)
    exit_reasons: list[dict] = Field(default_factory=list, max_length=12)
    benchmark_return: float | None = None


@router.post("/strategy/diagnose")
async def strategy_diagnose(req: DiagnoseRequest):
    """单策略回测后诊断: 结合配置(触发器/止损/持有)、指标、出场原因分布与基准收益,
    让 AI 指出亏损死因与具体参数修改建议。字段白名单过滤后入提示词。"""
    from app.services.ai_provider import ai_configured, generate_ai_text

    if not ai_configured():
        raise HTTPException(status_code=400, detail="未配置 AI, 无法生成诊断")

    cfg_keep = ("description", "entry_signals", "exit_signals", "stop_loss",
                "take_profit", "trailing_stop", "max_hold_days", "score_min")
    stats_keep = ("win_rate", "profit_factor", "total_return", "max_drawdown",
                  "n_trades", "avg_pnl", "avg_win", "avg_loss")
    er_keep = ("reason", "count", "avg_pnl")
    payload = {
        "策略名": req.name,
        "回测窗口": req.window_label,
        "股票池范围": req.scope_label,
        "配置": {k: req.config.get(k) for k in cfg_keep},
        "指标": {k: req.stats.get(k) for k in stats_keep},
        "出场原因分布": [{k: e.get(k) for k in er_keep} for e in req.exit_reasons],
        "同期基准收益": req.benchmark_return,
    }
    system = (
        "你是量化策略诊断助手。用户给出一个 A 股选股策略的配置与回测证据"
        "(比例均为小数; 出场原因 stop_loss=止损, max_hold=持有到期强平, "
        "exit_signal=出场触发器, 其余按字面理解)。请按四步输出中文诊断:\n"
        "1) 市场环境定性: 用策略收益 vs 同期基准收益判断是「策略问题」还是「环境问题」;\n"
        "2) 死因归析: 用出场原因分布指出亏损主要死在哪里(如止损被噪音打穿 / 出场触发器"
        "形同虚设导致持有到期强平), 引用具体数字;\n"
        "3) 具体改法: 明确到参数与数值(如止损 -5%→-8% 或改 ATR 止损、持有 20→10 天、"
        "出场触发器加什么), 每条一句话;\n"
        "4) 一句话下一步(如改完用 AI 修改调筛选逻辑后重新体检)。\n"
        "全文≤400字, 纯文本可用序号; 交易笔数<30 须先声明样本不足结论仅供参考; 不是投资建议。"
    )
    try:
        text = await generate_ai_text(
            [
