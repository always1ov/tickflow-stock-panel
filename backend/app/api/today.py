    ...
    for sym, c in cands.items():
        ...
        if geo:
            try:
                ...
                # [R200] 阶段判定。事件说的是"今天发生了什么", 阶段说的是"这只票
                # 整体走到哪一段了, 该盯什么" —— 后者才是能直接照着做的那一句,
                # 之前只有复盘弹窗有, 这里补上。纯函数, 零新增取数。
                chan_phase = _kg.phase(geo, kc.get("runs"))
            except Exception as e:  # noqa: BLE001
        ...
        o = {
            ...
            "ctx": {
                ...
                # [R279] **加速度的原始值也落下来** —— 只记不反馈, 不进分、不改判定。
                #
                # 为什么要多这一个字段: R278 量出界面分档(±0.05)与主升浪第⑤条
                # (a1 >= 0)之间有一条 **16.1% 的缝**, 而现在无从回答它该归哪边 ——
                # 台账只存了三档的码, 落进缝里的那些天全被压成同一个「steady」。
                #
                # **不落分箱而落原始值**: 现在定分箱边界又是一次没有证据的猜测,
                # 而原始值事后可以任意分箱, 信息不丢。它也不登记成分组维度
                # (连续值分不了组), 只是躺在 ctx 里等分析。
                #
                # **数据采集不可回溯** —— 今天不记, 几个月后想回答这个问题时
                # 缺的就是这一段。R16 的「注记标签落台账」是同一个理由:
                # 补不了历史, 先做。
                "accel_a1": ((geo or {}).get("accel") or {}).get("a1"),
...
# 不让通道位置一项压过量价本身。
# [R217] 这里原来有一张 `_VERDICT_SCORE` 表 —— 10 条通道档位配 +10 ~ −15 的
# 分值, **全项目零引用**。它是 v1 的遗留: 那时候通道档位真的加减分, R134 改成
# 「注记一分不加一分不减」之后表就没人用了, 却留在文件里。
#
# 危害不是占地方, 是**读代码的人会以为通道档位在参与打分**。这一轮已经在
# `_conflicts` 规则②、`PH_LAUNCHING`、`lib/button.ts` 上各栽过一次 ——
# 「没人用的代码看起来像在用」是这个仓库反复出现的同一类问题。删掉。
#
# 通道档位现在的去处: `_annotations()` 里的 `verdict` 注记(纯展示)、
...
def _build_overview(repo, engine=None) -> dict:
    ...
    # [R47] 通道档位: 给全部自选打标, 不只 cand_syms。[R134] 它现在是**注记**,
...
`注记` 里的主线归属、AI 信号、历史胜率、通道档位,**都不参与把握分**,它们只是背景。硬性规定:
