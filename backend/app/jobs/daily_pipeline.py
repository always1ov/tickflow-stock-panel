...
def run_now(
    ...
    # [R493/R496 · fork] 日K历史原地改写了: 登记过的衍生缓存统一作废(精确到票; 全量重建整类)
    from app.services import derived_caches
    if not enriched_exists or backward_extension:
        derived_caches.enriched_rewritten(repo.store.data_dir, "stock")
    elif affected_symbols:
        derived_caches.enriched_rewritten(repo.store.data_dir, "stock", affected_symbols)
    ...
    if capset.has(Cap.KLINE_DAILY_BATCH) and (pull_index or pull_etf):
        ...
        try:
            ...
            if pull_etf:
                ...
                # [R493/R496 · fork] 除权因子变了的 ETF: 登记过的衍生缓存统一作废
                if affected_etfs:
                    from app.services import derived_caches
                    derived_caches.enriched_rewritten(repo.store.data_dir, "etf", affected_etfs)
    ...
    if not _prefs_regime.get_pipeline_regime_enabled():
        ...
        try:
            ...
            # [R506 · fork] 作者在这里推「情绪周期阶段切换」通知(切入退潮/冰点按 warn 级)。
            # 撤了: 情绪周期看的是涨停梯队, 衡量打板情绪, 用户试用一段时间后判定对趋势持仓
            # 没用, 页面上那一组也撤了。留着推送等于每隔几天拿打板节奏提醒一次「该动了」——
            # 「主升→退潮」读起来像卖出理由, 正是 AGENTS 第 10 条「趋势没坏就不给卖出理由」的反面。
            # 阶段本身照算(regime 重算里 refresh_phase_labels 不动), 只是不再推。
        except Exception as e:  # noqa: BLE001
...
# [R435] 「个股 AI 信号批量定时」整个撤了(连同 SIGNAL_AI_JOB_ID、增量判据
# signal_needs_refresh 与它的 24 小时安全阀) —— AI 信号停用, 用户打算以后用
# 斐波那契二型重做。上面「今日总览 AI 定时」是另一件事, 不动。
...
def start_scheduler(repo: KlineRepository, capset: CapabilitySet) -> AsyncIOScheduler:
    ...
    # [R27 恢复] 今日总览 AI: 默认关, 用户在页面开启后才注册([R435] 个股 AI 信号批量已撤)
