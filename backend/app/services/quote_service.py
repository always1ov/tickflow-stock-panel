...
class QuoteService:
    ...
    def stop(self) -> None:
        ...
        # 这一行与上面那条修复互不相干: 一个管"开关态要不要落盘", 一个管
        # "内存里哪些 key 还忙着", 所以作者的修复照收, fork 这行照留。
    ...
    def disable(self) -> None:
        ...
        """
        self.stop()
        # 作者把"持久化关闭"从 stop() 挪到了这里 —— 只有用户主动关才落盘。
    ...
    def _fetch_quotes(self, *, final: bool = False, final_boundary_ms: int | None = None) -> bool:
        ...
        with self._fetch_lock:
            ...
            # [fork] 自选档只拉自选那几只; 全市场档走上游那条(并接住它新加的
            # final_boundary_ms —— 只取上游那版的话自选档就被抹掉了, 只取 fork
            # 那版的话 final 定版的边界判定在全市场档上失效)。
    ...
    def _maybe_send_webhook(self, rule_events: list[dict], engine) -> None:
        ...
        try:
            ...
            # [fork] 钉钉 + [上游 v0.2.3] 自定义 JSON webhook / 邮件
            ...
            # 五条通道全都没配置才跳过
            if not any((feishu_url, wecom_url, dingtalk_url, custom_url,
                        email_adapter.is_configured(email_config))):
                return
            ...
            for ev in rule_events:
                ...
                if email_adapter.is_configured(email_config) and "email" in channels:
                    ...
                    enqueued += 1
            # [R160] muted 这个计数不能丢: 没有它就看不出"今天有多少条是被焦点
            # 名单挡下的", 而那正是判断门槛松紧的唯一依据。上游新加的两条通道
            # 无需另加门控 —— 静音判定在上面 continue 掉了, 走不到这里。
