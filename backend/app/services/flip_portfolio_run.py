...
#
# [R357] 默认从 2 年收到 1 年。用户: 「不需要回溯那么多, 没多大意义, 极少我手动
# 去选择的时候看长期」。一年 ≈ 250 个交易日 ≈ **12 个月格**, 正好是逐月收益那
# 一排横着排得下的量; 两年排出来 24 格, 每格窄到读不出数。
#
# 上面那句"窗口太短说明不了任何事"**仍然成立, 只是它说的不是这件事**: 它护的是
# 胜率与最大回撤(对策略性格的估计, 要样本量), 而逐月收益是**对已发生的事的
# 陈述** —— 九月赚了多少就是赚了多少, 不因为只看了一年而变得不成立。
# 所以短窗口真正的代价落在「长期成绩」那一排上, 那一排的样本量提示原样还在。
DEFAULT_YEARS = 1
