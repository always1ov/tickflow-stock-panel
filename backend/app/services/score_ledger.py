...
#
# ## [R230] **降回 v2** —— 这是全仓库唯一一次版本号往回走, 说清楚为什么
#
# 用户: 「完整回退到上一个版本的评分系统」「你先追溯以前看看多久没变动过这个
# 打分系统」。追溯结果: `opportunity_score.py` 在 2026-09-01(R134)之后**整整
# 七天没人动过**, 直到 2026-09-08 19:13 的 R189 开始连改六次。R134 那一版就是
# 要回去的地方, 而它的版本号正是 2。
#
# 版本号往回走通常是错的 —— 但这里不是"改了口径又叫回老名字", 而是**打分代码
# 逐字节还原成了 R134 的那一份**(`git show 973b148` 对比通过)。既然算出来的
# 是同一个数, 它就该和 09-01~09-08 那七天的记录归在同一把尺子下面 —— 那七天
# 有真实样本, 换个新版本号等于白扔掉它们。
#
# v3 / v4 的记录(09-08 晚 ~ 09-09)因此从统计里退场, 记录本身留着, 读侧按
# `_version_of` 分档 —— 与当初 v2 退场时同一套机制, 方向反过来而已。
SCORING_VERSION = 2
# 归因维度中文名(v2, 当前)。v3/v4 的两轴与 v1 的八项加减保留在下面,
# 那两段时间的老记录还要按它们解读。
FACTOR_LABELS = {
...
# "说了句话但一分不参与打分"的标签 —— 通道档位、六态趋势、主线名次、龙虎榜。
...
def _by_label(rows: list[dict], recent_dates: set[str]) -> list[dict]:
    ...
    会失真, 用户会以为"通道档位天天都有", 其实多数日子它是 None。
    """
    out = []
