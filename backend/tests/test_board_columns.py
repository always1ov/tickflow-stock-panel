...
def test_R310_位置列竖排且整列自然成列():
    ...
    [R314] **改回靠中轴那一路 —— 因为格子里多了尺度标签。** 用户问「这个分数
    指的是短期通道的吗」, 两行于是各自报出自己的尺度(短期 / 短中长)。两个
    标签字数不等(2 字 / 3 字), 各自居中的话两行的数据部分会错开 —— 这正是
    R298/R307 反复治过的那个病。所以标签列右对齐、数据列左对齐, 整块 `mx-auto`
    居中。**居中的是这一块, 不是每一行。**
    """
    ...
    assert "grid-cols-[max-content_max-content]" in pos, "「位置」列没有那条中轴"
    assert pos.count("justify-self-end") == 2, (
        "两个尺度标签没有都靠右 —— 少一个那一行的数据就与另一行错开"
    )
    # 锚在**那个数所在的一整行**上, 不是最内层那个 span。
    # [R311] 数字外面套了一层定宽右对齐的槽, `tabular-nums` 于是落在外层
    # (CSS 继承, 效果不变)。原来的锚只看最内层 —— 一加壳就红, 而那是假红。
    num_line = pos[pos.index("{s.pos_cn}"):pos.index("Math.round(s.pct * 100)")]
    assert "tabular-nums" in num_line, (
        "位置那个数不是等高数字了 —— 整列的 /100 会上下错开(6 与 107 一窄一宽)"
    )
...
def test_R314_两行都报出自己是哪个尺度():
    """用户: 「这个分数指的是短期通道的吗, 你干脆就说清楚, 比如只看短期只显示短期」。

    **他问得对, 而且这一格自己给了误导**: 第一行是短期那一档的读数, 第二行
    三个字却是 短/中/长 —— 两行尺度不同, 却谁都没说自己是谁。更容易误会的是
    **第二行第一个字和第一行说的是同一件事**(`combo_code` 取的就是 s/m/l 三档
    的 `pos`), 只是精度不同。不标尺度, 「通道内 49/100」与它下面那个「中」
    看着像两个读数。

    这条正反各钉一次:
      · 正面 —— 两个标签都在, 而且**标签是说明不是读数**(压到最暗最小);
      · 反面 —— 第一行只许读短期那一档, 一个字的中/长都不许混进来。
    """
    pos = _cell("PositionCell")
    body = pos[pos.index("return ("):]
    assert ">短期<" in body, "第一行没说自己是短期 —— 用户问的就是这个"
    assert ">短中长<" in body, "第二行没说那三个字的顺序是 短→中→长"
    # 标签是说明不是读数: 比数据更小更暗, 否则它会跟读数抢注意力
    for lab in (">短期<", ">短中长<"):
        i = body.index(lab)
        tag = body[body.rindex("<span", 0, i):i]
        assert "text-[9px]" in tag, f"{lab} 的字号和读数一样大了 —— 它是说明"
        assert "text-muted/4" in tag, f"{lab} 没压暗 —— 它会跟读数抢注意力"

    # **反面: 第一行只读短期那一档。** 这是「只看短期只显示短期」的字面要求 ——
    # 混进中/长的话标签就成了假话, 而且不会有任何东西报错。
    head = body[body.index(">短期<"):body.index(">短中长<")]
    for bad in ("kc?.m", "kc?.l", "kc.m", "kc.l"):
        assert bad not in head, f"第一行里混进了别的尺度: {bad}"


def test_R311_位置那个数写成刻度不写成百分号():
    ...
    **`6%` 在一张股票表里默认被读成涨跌幅** —— 隔壁「现价/涨跌」那一列印的
    ...
    改成 `N/100`: 刻度本身摆在脸上(0 在下轨、100 在上轨), 没有第二种读法。
    轨外那两档反而读得更顺 —— `-4/100`、`107/100`; 换成 `%` 时这两个数最容易
    被当成暴跌暴涨。

    **正文里不许再出现 `%`。** 悬停里也统一成同一个记法 —— 同一个数两种写法,
    是这仓库反复在治的那种病(R306「同一层判定只许一个名字」的同族)。
    """
    pos = _cell("PositionCell")
    body = pos[pos.index("return ("):]
    assert "/100" in body, "位置那个数没写成刻度 —— 光一个数字看不出量程"
    assert "}%" not in body and "%<" not in body, (
        "位置那个数又印成百分号了 —— 它会被读成涨跌幅"
    )
    # 整列对齐: 五个档位名都是三个字, 数字再给一个定宽右对齐的槽, /100 才成一条线
    assert "text-right" in body, (
        "数字没有右对齐的定宽槽 —— 6 / 47 / 107 一窄一宽, 整列的 /100 会锯齿状"
    )
    # 层次: 档位名最亮 > 数 > 刻度。刻度不许和数一样重
    tail = body[body.index("/100") - 200:body.index("/100")]
    assert "opacity-4" in tail or "opacity-3" in tail, (
        "`/100` 和数字一样重了 —— 它是刻度不是读数, 166 行重复这么多次会盖过那个数"
    )
    # 悬停也是同一个记法
    tip = pos[:pos.index("return (")]
    assert "/100" in tip, "悬停里还写着百分号 —— 同一个数两种写法"
