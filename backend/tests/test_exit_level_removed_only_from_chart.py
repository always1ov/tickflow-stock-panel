...
顺手把底下那套也删了, 而它还在撑着三个活的展示面:
    ...
    AI 信号里的持仓上下文(纪律第 7 条) stock_signal.py 自己那一支
...
def test_R403_三个活的展示面一个没少():
    ...
    assert "exitLines" in code_of("components/stock-analysis/WatchlistDecisionBoard.tsx"), \
        "决策台不再用出场线"
    sig = (BACKEND / "services" / "stock_signal.py").read_text(encoding="utf-8")
    assert "position_exit" in sig, "AI 信号里的持仓上下文没了(纪律第 7 条靠它)"
