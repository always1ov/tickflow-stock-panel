...
def _with_shallow_dip() -> pl.DataFrame:
    ...
    这份序列是搭出来的(R476 换过一次, 见下), 让粗/中/细分别认出 1 / 3 / 4 个锚。
    """
    # [R476] 换数据: 锚只在上攻段里找之后, 原来那份的回调大多落在上攻起点之前, 中细两档一样。
    # 这份在上攻途中放了三种宽度的回调: 宽的(6.8)粗档也认得出、中等的(8.6)中档起、窄的(11.2)
    # 只有细档认得出 —— 粗 / 中 / 细分别是 1 / 3 / 4 个锚。
    lows = ([10.0] * 8 + [10.0, 9.0, 8.0, 7.0, 6.0, 5.0] + [5.6, 6.2, 6.8, 7.4, 8.0]
            + [7.6, 7.2, 6.9, 7.3, 7.7, 8.1, 8.5] + [8.9, 9.3, 8.9, 8.6, 9.2, 9.6, 10.0, 10.4]
            + [10.8, 11.3, 11.7, 11.2, 11.9, 12.3, 12.7, 13.1, 13.5, 13.9, 14.3, 14.7])
    lows = lows + [lows[-1] - i * 0.45 for i in range(1, 5)]
