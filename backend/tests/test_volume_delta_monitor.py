...
def test_volume_delta_cooldown_suppresses_repeat():
    eng = MonitorRuleEngine()
    eng.set_rules([_rule(cooldown_seconds=300)])
...
def test_volume_delta_metric_amount():
    ...
    assert events[0]["volume_delta"] == 100.0
    assert events[0]["volume_delta_amount"] == 6e6


def test_volume_delta_amount_missing_degrades_silently():
    eng = MonitorRuleEngine()
    eng.set_rules([_rule(metric="amount", threshold_amount=5e6)])
    df = pl.DataFrame({
        "symbol": ["S1.SH"],
        "close": [10.0],
        "_volume_delta": [20_000.0],
        "_volume_delta_span": [6.0],
    })
    assert eng.evaluate(df) == []


def test_volume_delta_rule_edit_resets_cooldown():
    eng = MonitorRuleEngine()
    df = _df([{"symbol": "S1.SH", "_volume_delta": 12_000.0}])
    eng.set_rules([_rule(threshold_volume=9_000)])
    assert len(eng.evaluate(df)) == 1
    assert eng.evaluate(df) == []
    eng.set_rules([_rule(threshold_volume=8_000)])
    assert len(eng.evaluate(df)) == 1
...
def test_volume_delta_configured_filter_missing_column_fails_closed():
    eng = MonitorRuleEngine()
    eng.set_rules([_rule(basic_filter={"market_cap_min": 20e8})])
    assert eng.evaluate(_df([
        {"symbol": "UNKNOWN.SH", "_volume_delta": 20_000.0},
    ])) == []
...
def test_delta_open_protection(monkeypatch):
    ...
    assert qs._volume_delta == {"S1.SH": (10000.0, None)}
...
def test_delta_not_continuous_trading(monkeypatch):
    ...
    assert qs._volume_delta == {}
    assert qs._prev_stock_volume == {"S1.SH": (99999.0, None)}


def test_delta_long_gap_is_treated_as_resume_baseline(monkeypatch):
    qs = _qs(monkeypatch)
    qs._interval = 6.0
    qs._update_volume_delta([{"symbol": "S1.SH", "volume": 1000}], 1000.0)
    qs._update_volume_delta([{"symbol": "S1.SH", "volume": 99_000}], 61_000.0)
    assert qs._volume_delta == {}
    qs._update_volume_delta([{"symbol": "S1.SH", "volume": 100_000}], 67_000.0)
    assert qs._volume_delta == {"S1.SH": (1000.0, None)}


def test_delta_missing_amount_does_not_fabricate_amount_delta(monkeypatch):
    qs = _qs(monkeypatch)
    qs._update_volume_delta([{"symbol": "S1.SH", "volume": 1000}], 1000.0)
    qs._update_volume_delta(
        [{"symbol": "S1.SH", "volume": 2000, "amount": 9e6}],
        7000.0,
    )
    assert qs._volume_delta == {"S1.SH": (1000.0, None)}
