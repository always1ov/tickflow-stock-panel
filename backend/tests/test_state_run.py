...
def test_R256_三档都在中部那一格也有名字():
    ...
    [R308] 判定徽标撤了, 这一格改印**三字码本身**「中中中」。
    [R315] 码进悬停, 改成画一条轨 + 三个点 —— 用户说「更加看不懂了」。
    [R316] 改成**位置名 + 一句能交易的距离**。

    **病一次都没回来, 而且这一版最直白**: 三档都在中部时, 短期那一档的位置名
    就是「通道内」—— 一个每天都在用的词, 后面还跟着「距上轨 5.4%」这种能直接
    下单的数。它跟另外 26 格用的是同一套写法, 不是给这一格开的特例。

    所以这条守的东西一个字没变: **这一格不许是空的**, 而且不许把"位置上没得说"
    与"算不出来"混成一件事(R294 钉过)。
    """
    import pathlib
    ...
    assert "{s ? s.pos_cn : '—'}" in pos, (
        "「中中中」那一格又空了 —— 三档都在中部时 `verdict` 本来就是 null, "
        "只有位置名还认得出这一格"
    )
    assert "{near.label}" in pos, "距离那一行没了 —— 这一格会只剩一个孤零零的词"
    # 反面: **「都在中部」与「算不出来」必须长得不一样。** 前者印「通道内 距…」,
    # 后者两行都是 `—`(R294: 数据缺失不许被读成"这是个稀有位置")。
    assert pos.count("text-muted/30") == 2, (
        "「算不出来」那一路的占位不全 —— 它会和「通道内」混成一件事"
    )
