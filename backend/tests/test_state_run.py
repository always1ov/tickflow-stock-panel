...
def test_R256_三档都在中部那一格也有名字():
    ...
    [R315] 码也进悬停了, 这一格改成**画**: 一条轨 + 三个点。

    **病一次都没回来, 而且这一版最彻底**: 三档都在中部时, 三个点会挤在轨的
    正中间 —— 那是一幅**看得见的图**, 比「通道中部」四个字、比「中中中」三个
    字都更直接地说出了"它们都在中间"这件事。而且它跟另外 26 格用的是同一套
    画法, 不是给这一格开的特例。

    所以这条守的东西一个字没变: **这一格不许是空的**。守法跟着从"有没有那几个
    字"换成"有没有画那几样东西"。
    """
    import pathlib
    ...
    # 三档都在中部 → 三个点都落在轨的正中间。这一格靠**画**说话, 所以钉的是
    # "轨与点都画得出来", 而不是"印了哪几个字"。
    assert "{(['l', 'm', 's'] as const).map" in pos, (
        "「中中中」那一格又空了 —— 三档都在中部时 `verdict` 本来就是 null, "
        "只有这三个点还认得出这一格"
    )
    assert "bg-border/70" in pos, "轨没了 —— 三个点挤在中间这件事就失去了参照"
    # 反面: 某一档算不出来只该少一个点, 不该让整格塌掉(R294: 数据缺失与
    # "位置上没得说"是两件事)
    assert "if (!b) return null" in pos, (
        "缺一档时整条轨会塌 —— 那会把「算不出来」和「都在中部」混成一件事"
    )
