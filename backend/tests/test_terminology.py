...
TERMS: list[tuple[str, str, tuple[str, ...]]] = [
    # 概念,      正名,       不许出现的别名
    # [R306] 正名从「通道结论」改成「通道档位」。用户: 「结论这个名称改成位置」;
    # 问到撞名时选了「通道档位」。
    #
    # **为什么不叫「通道位置」**(那是第一版方案): 打分系统里 `channel_pct`
    # (短期通道里的位置百分比 0~100)**已经叫这个**, 而它在逐字节冻结的
    # `opportunity_score.py`(R134)里改不了 —— 照那个方案改, 体检面板会显示
    # 「通道位置 72」, 复盘会显示「通道位置: 大顶区域」, 同一个名字两件事,
    # 正是这次改名要除掉的毛病。**这张表没抓到它**(那两处一个在冻结模块、
    # 一个是 dict 键), 是人工核对时发现的 —— 下面补了一条专钉这次撞名。
    #
    # 「档位」也更贴这一层的实体: 界面上本来就满处是「这一档」「十档」「换档」
    # 「已N天」, 它是十个档位, 不是一个位置。
    ("三档通道位置合成的那一档判定", "通道档位", ("贵不贵", "位置结论", "通道结论")),
    ("三档各在上中下拼成的那个三字码", "三档组合", ("位置组合", "位置码")),
    ...
    ("六态状态今天翻转了", "转折", ("刚变盘",)),
    # [R373] 菜单里那个展示型分组的标签。用户原话: 「菜单的盘面参考改名成闲置功能」。
    # **只动 label/hint/渲染文本**: 内部 id(`group:browse`)、常量名、变量名、
    # localStorage key(`tf-nav-browse-open`)都保持不变 —— 那是用户的存储键,
    # 改了会丢已存的顺序与折叠状态。
    ("菜单里那个展示型分组的标签", "闲置功能", ("盘面参考",)),
    # [R404] 回撤位那一组的组名从「斐波那契」改成「斐波那契一型」。
    #
    # 起因是**马上要加第二套**(「斐波那契二型」, 帝纳波利点位)。两套都是回撤位,
    # 但算法完全不同:
    #   一型 = 近 120 日里**一个**波段(窗口内最高/最低)的 0.236~0.786;
    #   二型 = 推进段最高点配最多 5 个反应点, **每一对**都算 38.2%/61.8%,
    #          价值在于哪几条挤在一起。
    # 留一个光秃秃的「斐波那契」在界面上, 读的人不知道指的是哪一套。
    #
    # **别名写成正则不是图花哨**: 这张表是纯子串匹配, 而正名「斐波那契一型」
    # 自己就含着「斐波那契」—— 写成字符串别名会把正名判成违规, 这条改名就
    # 根本登记不上。见 `find_hits`。
    ("近 120 日单波段那组回撤位的组名", "斐波那契一型",
     (re.compile(r"斐波那契(?![一二]型)"),)),
    # [R410] 斐波那契二型第一版那套界面名**没让人看懂**。用户原话:
    # 「目标1目标2失效位这些表达没能让用户抓得住重点看得懂, 而且好多根线,
    #   好难抓住之前说的做不做在哪里做, 走不走这些」。
    #
    # 四处改名, 每处都是**撞名或误导**, 不是换个好听的说法:
    #
    #   · 「浅/深回撤」→「浅/深回踩」。**「回撤」在本仓库已经有两个别的意思,
    #     而且都动不了**: ① 账户/组合口径(最大回撤、蒙卡回撤、组合回撤纪律线、
    #     回撤止盈); ② 六态里作者命名的「自然回撤 / 次级回撤」(R405 之后作者
    #     命名一律不动)。同一个词第三个意思 —— 让最新来的这组让路。
    #   · 「强支撑区」→「回踩密集带」。「支撑」已经被「压力支撑」那一组占着。
    #   · 「失效位」→「这组作废」。「失效」太抽象, 而且**容易被读成一条卖出线**,
    #     它不是 —— 它只说明跌破之后这一整组位置不再成立。
    #   · 「目标一二三」→「第一站/第二站/第三站」。「目标」在本仓库还指目标仓位
    #     与目标日波动; 而且「目标」听着像"要去达成的", 这三条只是推算出来的、
    #     涨上去会路过的位置。
    #
    # 「目标一」写成正则是因为 `目标一致` 这个正常词组在后端注释里有三处 ——
    # 纯子串会把它判成违规(见 `find_hits` 那条关于正则别名的说明)。
    ("二型里浅的那一档回踩位", "浅回踩", ("浅回撤",)),
    ("二型里深的那一档回踩位", "深回踩", ("深回撤",)),
    ("二型里几条回踩位挤在一起的那个带", "回踩密集带", ("强支撑区",)),
    ("跌破之后二型整组不再成立的那条线", "这组作废", ("失效位",)),
    ("二型往上推算的第一个位置", "第一站", (re.compile(r"目标一(?!致)"),)),
    ("二型往上推算的第二个位置", "第二站", ("目标二",)),
    ("二型往上推算的第三个位置", "第三站", ("目标三",)),
]

# 核查过、**确认不是冲突**的同形词 —— 写下来免得下次又被当成问题重查一遍。
NOT_A_CONFLICT = {
    "今日总览(后端剩下的两处)": (
        "[R393] **这是个真冲突, 但剩下那两处不在界面上, 所以没有进 `TERMS`。** "
        "那一页 R340/R351 拆了并重定向到模拟盘, 前端渲染文本已经清空(由 "
        "`test_score_ledger_silent_skip.py` 整个 `frontend/src` 扫着), 数据体检的 "
        "标签也改成了「总览门槛偏好」。剩下的两处都在 `api/today.py`: 一处是**送给 "
        "模型的提示词**, 一处是 payload 的**键名** —— 后者按 [R121] 同时是校验 AI "
        "有没有忠实使用我们喂的数据的账本, 改键等于动核心逻辑。用户对这一轮划的 "
        "范围是「渲染前端可以动的, 核心逻辑不允许」。"
        "**所以别给它加 `TERMS` 行**: 加了会当场把这两处判成违规, 而唯一的消解办法 "
        "是去动不许动的东西, 或者把 `api/today.py` 整个塞进 `SKIP_FILES` —— 那会 "
        "连它里面真正的界面文案(自检条那几个登记名)一起不扫, 比不加更坏。"
    ),
    "转折后第N天/已N天": (
        "[R290] **两个锚点, 不是两个说法**。走势列的「转折后第 N 天」从**六态转折那天**"
        "数起(用户点名要这个说法: 「这类词统一改成出现转折后的第几天」); 结论徽标的"
        "「已N天」是**这一档通道档位**连着多久 —— 两段的起点根本不是同一天, 合成一个"
    ...
    "通道态势": (
        "**这一条是这张表自己抓到的**: 第一版把它当成「通道档位」的旧名收进了别名表, "
...
def find_hits(words, rendered) -> list[str]:
    ...
    [R404] 别名可以是字符串(子串匹配, 原样), 也可以是**编译好的正则**。

    加正则这一路是被逼的: 这张表原本纯子串匹配, 而有一类改名**新名字自己就
    含着旧名字** —— 「斐波那契」→「斐波那契一型」就是。写成字符串别名的话,
    守卫会把正名本身判成违规, 等于这条改名根本没法登记。正则能写
    `斐波那契(?![一二]型)`, 只抓光秃秃的那个。
    """
    be, fe = rendered
    out: list[str] = []
    for w in words:
        pat = w if isinstance(w, re.Pattern) else None
        shown = pat.pattern if pat else w
        for side, rows in (("后端", be), ("前端", fe)):
            for f, i, v in rows:
                if (pat.search(v) if pat else w in v):
                    out.append(f"[{side}] {f}:{i} 用了「{shown}」")
    return out
...
def test_R404_正则别名这一路真的在查(rendered):
    """**这一条守的是"守卫自己哑掉"。**

    R404 给别名加了正则这一路。变异测试当场证明它有个安静的坏法: 把
    `pat.search(v)` 写回纯子串匹配之后, 拿来比的就成了**正则的源码文本**
    (`斐波那契(?![一二]型)`), 那串字当然哪儿都不会出现 —— 于是一条命中都没有,
    整组**全绿**。守卫不是报错, 是不吭声了。

    做法与 `test_R279_两侧都真的接在检查里` 同一个形状: 喂一个**已知一定命中**的
    正则进去, 它必须真的抓到。
    """
    # 「斐波那契一型」在两侧都在(组名 + 前端开关), 这个正则必须抓得到它
    hits = find_hits((re.compile(r"斐波那契[一二]型"),), rendered)
    assert hits, "正则别名这一路没在查 —— 已知存在的词都抓不到"
    # 反过来: 正名不该被"光秃秃的斐波那契"那条抓到(否则改名根本登记不上)
    bare = find_hits((re.compile(r"斐波那契(?![一二]型)"),), rendered)
    assert not bare, f"正名被自己的别名规则判成违规了: {bare}"
...
def test_R279_这道闸真的扫得到后端():
    ...
    hit = [v for f, _i, v in be
           if f.endswith("keltner_geometry.py") and "再争论「通道档位」没有意义" in v]
