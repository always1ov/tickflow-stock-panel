...
TERMS: list[tuple[str, str, tuple[str, ...]]] = [
    ...
    # 根本登记不上。见 `find_hits`。
    #
    # [R446] 「一型 / 二型」→「Ⅰ型 / Ⅱ型」。用户: 「斐波那契一型二型换成罗马数字」。
    # 正则跟着改成只放过罗马数字 —— 于是旧名「斐波那契一型 / 二型」和光秃秃的
    # 「斐波那契」都会被这一条抓到。「二型均线」另登一条。
    ("近 120 日单波段那组回撤位的组名", "斐波那契Ⅰ型",
     (re.compile(r"斐波那契(?![ⅠⅡ]型)"),)),
    ("推进段最高点配反应点的那组回撤位的组名", "斐波那契Ⅱ型", ("斐波那契二型",)),
    ("Ⅱ型那条 3 日均线后移 3 根", "Ⅱ型均线", ("二型均线",)),
    ...
    # [R412] 「第一站/第二站/第三站」只活了一轮就又改了。用户提的是
    # 「阶段止盈点一/二」, 而**本仓库已经有「止盈线」**(`position_exit.py` 的
    # ATR 三阶段吊灯止盈 —— 决策台有一列、会触发盘中推送、优先级排在生命线下面)。
    # 同一屏上两个不同体系的「止盈」数字, 正是这张表在防的事; 用户听完选了
    # 不撞名的那一版。**新名字把来源写进了名字里**: 它是由这一波上攻推算出来的。
    ("二型往上推算的第一个位置", "上攻推算位一", (re.compile(r"目标一(?!致)"), "第一站")),
    ("二型往上推算的第二个位置", "上攻推算位二", ("目标二", "第二站")),
    ("二型往上推算的第三个位置", "上攻推算位三", ("目标三", "第三站")),
    # [R503] 菜单「市场环境」改名「宏观分析」(用户原话), 页上两组同时铺开。
    # **只收「市场环境页」这一个别名**: 「市场环境」本身仍是那一组内容、数据页那张卡片、
    # 回测「市场环境过滤」的正名(regime 数据), 收进来会把这些合法文案全判成违规。
    ("/regime 那一页", "宏观分析", ("市场环境页", "「市场环境」页")),
    # [R512] 模拟盘分成五栏(用户: 「模拟盘的内容分类整理成图片这样的表达方式」), 栏名定下来后
    # 栏里卡片的标题跟着改成同一个名字(用户选的「按推荐」)。原来一样东西散着三四个叫法:
    # 分栏上叫「持仓」, 卡片叫「现在拿着」, 信号栏里那一段叫「手上这些」, 统计格又叫「现在拿着」。
    # [R518] 用户: 「今日信号改成信号」—— 正名再改一次, 旧名收进别名列
    ("模拟盘的信号那一栏", "信号", ("今天该挂什么单", "今日信号")),
    ("模拟盘手上拿着的票", "持仓", ("现在拿着", "手上这些")),
    ("模拟盘那套打法回测出来的成绩", "成绩", ("这套打法的成绩",)),
    ("模拟盘的成交记录", "流水", ("成交流水",)),
    # [R513] 今日信号重做成三段(要动手 / 持仓 / 盯着), 第三段原来叫「只是盯着」。
    ("模拟盘今日信号里没拿着、还没到触发价的那一段", "盯着", ("只是盯着",)),
    # [R524] 监控中心那一条「推送焦点」成了一栏, 名字改回 CONTEXT.md 与后端一直在用的「焦点名单」。
    # 界面上曾同时有「推送焦点」(条的标题)、「只认焦点名单」(开关)、「只看焦点」(记录筛选) ——
    # 三个词指同一份名单。「只看焦点」是动作不是名词, 留着。
    ("自选里值得推送的那份名单", "焦点名单", ("推送焦点",)),
    # [R533] 设置页分栏名去掉重复的「设置」二字(页标题已经是「设置」): 界面上提到这几栏一律写「设置 → AI」这种。
    # 别名只收这三个 —— 它们除了当栏名没有别的意思; 「实时监控」「数据源」「扩展页面」没改名, 不收。
    ("设置页的 AI 那一栏", "AI", ("AI 设置",)),
    ("设置页的网络那一栏", "网络", ("网络设置",)),
    ("设置页的系统那一栏", "系统", ("系统设置",)),
]
...
NOT_A_CONFLICT = {
    "26日布林 / 20日布林上轨 / 突破布林上轨": (
        "[R528] **两个数, 两个名字, 不是同一个东西的两种叫法。** 图表(关键价位图、日K图)画的布林线"
        "是 26 日 ±2σ(用户: 「改成 26 日布林线, 名称也是对应的表达」), 产地 `indicators/bollinger.py`; "
        "enriched 表的 `boll_upper` / `boll_lower` 仍是 20 日 —— 作者内置策略「布林突破」读的就是它们, "
        "不能动。所以凡是印那两列的地方(自选 / 策略结果表的列、条件字段名)都写成「20日布林上轨」, "
        "图表的写成「26日布林上轨」(周期在前, 用户 R529 定的写法), 一眼分得开。信号「突破布林上轨」「跌破布林下轨」"
        "说的是 20 日那组上的事件, 名字里没数是因为它是事件名不是价位名, 且策略描述 / 监控消息里"
        "已经用了很久; 由 `test_boll26_chart` 钉着这三组各自的写法。"
    ),
    "今日总览(后端剩下的两处)": (
...
def test_R404_正则别名这一路真的在查(rendered):
    ...
    (`斐波那契(?![ⅠⅡ]型)`), 那串字当然哪儿都不会出现 —— 于是一条命中都没有,
    整组**全绿**。守卫不是报错, 是不吭声了。

    做法与 `test_R279_两侧都真的接在检查里` 同一个形状: 喂一个**已知一定命中**的
    正则进去, 它必须真的抓到。
    """
    # 「斐波那契Ⅰ型」在两侧都在(组名 + 前端开关), 这个正则必须抓得到它。
    # [R446] 一型 / 二型 → Ⅰ型 / Ⅱ型, 两个正则跟着换
    hits = find_hits((re.compile(r"斐波那契[ⅠⅡ]型"),), rendered)
    assert hits, "正则别名这一路没在查 —— 已知存在的词都抓不到"
    # 反过来: 正名不该被"光秃秃的斐波那契"那条抓到(否则改名根本登记不上)
    bare = find_hits((re.compile(r"斐波那契(?![ⅠⅡ]型)"),), rendered)
