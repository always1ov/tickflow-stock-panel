...
# ================================================================
# [R288] 通道档位那一栏 —— 同一台发动机, 换一套「什么时候该有仓位」
# ================================================================
#
# 用户: 「通道结论这个部分也能这样搞类似的统计吗」。
#
# 能, 但**比六态多一个难点**: 六态每天非多即空, 而通道档位有三种
...
def test_R288_结论那一栏与页签上的卡片一一对应(review):
    ...
    「通道档位」页签的卡片是按 `verdict.code` 分段的, 所以每一笔的 `flip_date`
    都得是逐日行里**结论换了一档**的那一天。
    """
...
def test_R288_结论那一栏买卖比六态频繁(review):
    """**这本身就是个结论**, 不是巧合: 通道档位天天在变, 六态是趋势级的。
...
def test_R288_两个页签都挂上了而且都排在正文最前面():
    ...
    # [R295] 通道档位的正文也换成了逐日表(用户: 「通道结论那部分也想要这样的
...
def test_R288_两栏的口径各写各的():
    ...
    assert 'label="按档位买卖"' in dlg, "结论那栏的栏目名没了"
...
def test_R288_按清空模拟这件事要说出来():
    ...
    # 光有这段文字不够 —— 它得真的渲染在「通道档位」那一节里, 而不是躺在
    # 源码某处。锚在那一节的 `Section` 上, 顺序也钉住。
    i = view.index('title="通道档位"')
    assert view.index("按档位买卖的口径", i) > i, "那段话没落在「通道档位」那一节里"
...
def test_R292_那些不要的东西真的删干净了():
    ...
    # 只有「趋势状态」那块折叠区退役了。「通道档位」那条「依据」还在(见上面
...
def test_R302_通道档位那一页头一个说的就是今天这一档():
    """**这一页叫「通道档位」, 而它的头部卡里原来根本没有今天那一档结论。**

    查废话时才发现的(用户: 「检查通道档位页面有没有废话」「都围绕位置展开」)——
...
def test_R302_不随票变的话不许常驻正文():
    ...
    **判据是"随不随票变"**, 不是"重不重要": 「按档位买卖」的口径偏差(「拿着」
...
# ================================================================
# [R293] 「通道档位」照着「趋势状态」重排
# ================================================================
#
# 用户: 「通道结论也复刻参考趋势状态改好的排版显示重点内容, 通道结论这部分的
# 关注重点是『调整到位』和这些状态期间的买卖」;「你可以理解为核心是按档位买卖」;
# 「你别搞错了, 趋势状态也是按出现转折买卖而且已经做好了不用再改了, 通道档位
# 才是按档位买卖, 这是两个不同的核心」。
...
def test_R293_两个页签是两个不同的核心():
    ...
    趋势状态吃 `flip_trades`(按六态**转折**买卖), 通道档位吃 `verdict_trades`
    ...
    assert 'label="按转折买卖"' in trend and 'label="按档位买卖"' in verdict
...
def test_R293_两个页签的头部是同一个形状():
    ...
    """
    dlg = _dialog()
    # [R294 → R296] R294 时是三个页签; R296 把「组合速查」并进「通道档位」,
    # 剩两个 —— 规矩没变, 覆盖面跟着少一个。
...
def test_R295_每一笔长在换档那一行上():
    ...
    R293 并进的是段落卡片; R295 用户指着「趋势状态」那张逐日表说「通道档位那
...
def test_R293_这N天那一行按语气分档而不是十档全铺():
    ...
    # [R306] 「N 段结论」→「N 段档位」, 跟着改名走
    assert "{segments.length} 段档位" in verdict, "没给总段数"
...
# 用户: 「组合速查也要, 它是按照位置为核心」。
#
# 照前两个页签的样子, 第三栏该是「按三档三档组合换格买卖」。**但那个数会和
# 「按档位买卖」一模一样** —— 而这个仓库最不该做的就是把同一个数印两遍。
#
# 原因是: 方向(该不该持仓)是**逐日**由那一格的语气定的, 而三档组合换格比结论
...
def test_R294_多切几刀不改变成绩(real):
    ...
    **前提是切在首尾之间**(见 `_resegment` 的说明): 切到第一次转折之前会换掉
    ...
    「按位置买卖」与「按档位买卖」唯一会差出来的地方, 差的只是那一头一尾。

    所以「按位置买卖」印出来会与「按档位买卖」逐字相同, 那是同一个数两个名字
...
def test_R296_位置并进了通道档位那一页():
    ...
    **本来就该合**(`test_R296_结论是位置的纯函数` 穷举 125 种三档位置证过:
    ...
    assert "comboHistory(d.rows, here)" in now, "「这一格历来」没并到通道档位页"
    assert "d.channel?.geo?.combo" in now, "「你在哪一格」没并到通道档位页"
...
def test_R296_27格进了说明那一页而不是另外两页():
    ...
    **那个取舍的交代跟着搬**: R294 在「组合速查」页上印过一句"为什么这一页不摆
    战绩"(换格两边同向, 复利乘回去与按档位买卖逐字相同)。那一页没了, 问题还在 ——
...
def test_R296_这一格历来按段不按天():
    ...
    # 前瞻取**段末**那天 —— 与「通道档位」那边同一个道理
    assert "!next || next.combo !== here" in fn, "前瞻收益取的不是段末那天"


def test_R294_组合码来自后端不在前端拼():
    """拼法归 `combo_code` 管。前端再拼一份就是同一个规则两处定义(R286)。"""
    combo = _combo()
    assert "r.combo" in combo, "没用后端给的组合码"
    # 反面: 不许在这里用 bands 现拼一个三字码
    for hand in ("'上' :", "? '上'", "join('')"):
        assert hand not in combo, f"像是在前端手拼组合码: {hand}"


def test_R296_没进过这一格与算不出来分得开():
    """[R294 → R296] 规矩一个字没变, 只是搬到了「通道档位」那一页上。
    ...
    assert "{!!here && (" in now, "「这一格历来」没有挂在组合码上"


# ================================================================
# [R295] 「结论」列离开趋势表, 通道档位拿到自己的逐日记录表
# ================================================================
#
...
def test_R296_趋势表里的结论列恢复了():
    ...
    回来时有三处必须一起对上, 少一处就歪:
      ...
      ② 列头逐字叫「通道档位」(R258: 同一层判定只许一个名字);
      ③ 空表那行的 `colSpan` 跟着回到 8。
    """
    trend = _fn_body(_dialog(), "function TrendView")
    assert "<VerdictHover v={r.verdict}" in trend, "「通道档位」那一列没回来"
    assert trend.index("<FlipTradeCells") < trend.index("<VerdictHover"), (
        "结论那一列插进了成交三格前面 —— 旁证不该打断主线"
    )
    assert ">通道档位</th>" in trend and ">结论</th>" not in trend, (
        "列头没叫「通道档位」—— 两张表并排放着会是同一样东西两个名字(R258)"
    )
    assert "colSpan={8}" in trend, "加回一列却没改 colSpan"


def test_R295_通道档位有一张同形状的逐日表():
    ...
    assert "通道档位</th>" in verdict, "结论那一列没进新表"
    assert "六态状态</th>" in trend and "六态状态</th>" not in verdict, "两张表的状态列串了"
    # [R296] 列数不再相等: 趋势那张多一列「通道档位」(用户要它回来), 通道那张
