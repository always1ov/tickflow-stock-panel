"""[R269] 复盘·通道档位栏的版面 —— 结论在上, 依据收起, 正文占满。
...
def test_R269_位置结论压成一枚芯片(dlg):
    ...
    [R279] 这一栏的名字**从「位置结论」改成了「通道档位」** —— 同一个东西
    在这个弹窗里原来有三个名字(页签叫通道档位、逐日表列头叫结论、这里叫位置结论),
    而文件自己的注释都写着「这一层在别处一律叫通道档位」。守的规矩一个字没变:
    它得压成一枚芯片, 不许再占一整块。
    """
    # [R293] 「通道档位灵不灵」那枚芯片**撤掉了** —— 与六态那枚一起(R292)。
...
def test_R269_依据排在正文之前(dlg):
    ...
    """
    view = _fn(dlg, "function VerdictView")
    assert "<OutcomeChips" not in view, "分档芯片又回到「通道档位」页了"
    # [R293] 头部卡 → 依据 → 卡片流。头部卡的第一行就是「按档位买卖」(核心)。
...
def test_R273_每个页签都有时间轴(dlg):
    ...
    趋势状态一条(六态), 通道档位两条(合成结论 + 三档各自, 见下一条)。"""
...
def test_R273_通道档位沿用决策台那套配色(tlib):
...
def test_R273_三档各自那条与合成结论那条都在(dlg):
    ...
    [R296] 它原本长在「组合速查」页上; 那一页并进「通道档位」时, 这条带子跟着
    搬到同一张头部卡的「这 N 天」行里, **摆在合成结论那条下面**(先看结论怎么
    走, 再看它是从哪三档来的 —— 与「现在」行里组合码排在结论后面同一个顺序)。
    """
