...
def test_pipeline_self_heals_snapshot_day(tmp_path, monkeypatch):
    ...
    # [fork 适配] 先铺够一段正常历史(批量口径, quote_ts=None) —— 本 fork 的
    # 「历史稀疏检测」看到近一年分区太少会直接判大洞、改走一年重拉, 那样本测试
    # 想验证的"从坏日起修复"分支就永远走不到。这两个保护各管各的场景:
    # 稀疏管年尺度大洞, 本测试管最近几天的盘中快照。
    d = yesterday
    dense = 0
    while dense < 130:
        d -= timedelta(days=1)
        if d.weekday() >= 5:
            continue
        _write_full_partition(tmp_path, "kline_daily", d, None)
        dense += 1
