...
export function RuleEditor({ rule, preset, simple, onClose, onSaved }: Props) {
  ...
  const quoteInterval = quoteStatus?.interval_s ?? 6
  const volumeDeltaAvailable = quoteStatus?.mode === 'full_market'
      ...
      if (d.type === 'strategy') {
        ...
        if (d.metric === 'amount') {
          if (!Number.isFinite(d.threshold_amount) || (d.threshold_amount ?? 0) < 1) {
            throw new Error('金额阈值必须是至少 1 元的数字')
          }
        } else if (!Number.isFinite(d.threshold_volume) || (d.threshold_volume ?? 0) < 1) {
          throw new Error('单轮放量阈值必须是至少 1 手的数字')
        }
      } else {
        delete d.score_min
        delete d.score_max
            ...
            <label className="space-y-1.5">
              ...
              <input
                ...
                value={draft.metric === 'amount'
                  ? (draft.threshold_amount ?? 1e6) / 1e4
                  : (draft.threshold_volume ?? 9000)}
                onChange={event => setDraft(d => {
                  const value = Number(event.target.value)
                  return d.metric === 'amount'
                    ? { ...d, threshold_amount: value * 1e4 }
                    : { ...d, threshold_volume: value }
                })}
