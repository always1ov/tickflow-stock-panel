...
const GROUPS: { title: string; note?: string; items: Entry[] }[] = [
  ...
  // 讲的全是"这个形状代表什么"。
  {
    ...
    note: '底下所有的判定 —— 结论、通道态势、怎么办 —— 都是从这一件事推出来的。',
    items: [
      { term: '三条线 = 同一个价格的三种看法',
        meaning: '短期、中期、长期不是三个指标,是**同一只票的价格在三个时间尺度上各自的「合理区间」**。'
          + '短期那条问的是「按最近这一小段看,现在这个价算高还是算低」,长期那条问的是「按一大段看呢」。',
      ...
      { term: '分歧太大 = 已经没有共同的合理价',
        ...
        read: '这时候争论价位高低没有意义,得先想清楚你做的是哪一段。'
  ...
  {
    title: '怎么办(第一列)',
    note: '整张表唯一的收敛层 —— 把「该动了 / 六态趋势 / 结论 / 通道态势 / AI 信号」五套判定合成一句话。第一条命中即止。',
    items: [
      { term: '按纪律走',
        meaning: '出场线或生命线已经破了。',
  ...
  {
    title: '结论',
    note: '三条线的位置合起来读出的一句话。它说的是**位置**(这个价算高还是算低),不是方向 —— 要配着下面那行事件一起看。',
    items: [
      { term: '这一列答的是什么',
