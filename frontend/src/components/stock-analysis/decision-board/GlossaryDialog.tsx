...
const GROUPS: { title: string; note?: string; items: Entry[] }[] = [
  {
    title: '量化波动通道',
    note: '同一只票在三个时间尺度上的位置读数,竖排三行:短、中、长。三行同向说明三个尺度看法一致;打架说明它们在说不同的事。',
    items: [
      { term: '短 / 中 / 长',
        meaning: '这只票眼下的价格,在各自那个时间尺度里算高还是算低。',
        read: '短的一行对突发消息最敏感,长的一行只在真的换了阶段时才动。所以「短动而中长不动」多半是一次性冲击,三行齐动才是状态变了。' },
      { term: '结论',
  ...
  },
  {
    ...
    items: [
      ...
      { term: '趋势内加速 / 主升浪特征',
        meaning: '趋势已经确认之后的再上一个台阶;后者是几个条件同时成立的那种少见情形。',
        read: '这两种情况下沿着上沿走是常态,不必因为「到高位了」就减。' },
      { term: '末端钝化',
        meaning: '还在高位待着,但推动力在减弱。',
        read: '涨势没坏,只是没劲了 —— 该开始想退出计划,而不是加仓。' },
      { term: '反弹遇阻',
        meaning: '趋势本身是向下的,这次上冲只是撞到阻力。',
        read: '**最容易被读成突破的一种。** 看着像转强,其实方向没变。' },
      { term: '强势洗盘',
        meaning: '趋势还在多头一侧,但价格短暂跌破了下沿。',
        read: '和「破位」长得像,区别在于大尺度还站着。别把洗盘当破位割在底部。' },
      { term: '压缩待变',
        meaning: '三个尺度对「合理价」几乎没有分歧,方向未定。',
  ...
  },
  {
    ...
    items: [
      { term: '加速度',
        ...
        read: '**不是越大越好。** 适度加速是行情刚起来;过度加速往往出现在一波的末端,不是起点。' },
      { term: '分离度',
        meaning: '三个尺度之间拉开了多远,带方向 —— 正的是往上拉开,负的是往下拉开,接近零是挤在一起。',
        read: '接近零 = 方向未定;适度拉开 = 趋势立住了;拉得过开 = 已经走了很长一段,再追的性价比在下降。' },
      { term: '尺度撕裂',
        meaning: '拉开到了极端 —— 短期视角和长期视角已经没有任何共同的「合理价」。',
        read: '任何价格在一个尺度上算贵时,在另一个尺度上都算便宜。这时候争论「贵不贵」没有意义,得先说清楚你按哪个尺度做。' },
      { term: '已粘合 N 天 / 季度平均压缩',
        meaning: '前者是**连续**多少天三个尺度看法一致,后者是这个季度**平均**有多一致。',
        read: '**两个要一起看。** 连续天数为零但平均很高 = 刚刚启动;连续天数不短但平均不高 = 反复挣脱又缩回去。' },
      { term: '波动主导频段',
        meaning: '这只票眼下的波动,主要来自很短周期的跳动、一波行情的主体,还是更长期的老趋势。',
        read: '高频占优 = 更像消息和情绪在推;中频占优 = 一波行情正在主升段;低频占优 = 能量都在老趋势里,近期反而平静,动能在衰减。' },
    ],
  },
  {
    title: '该动了(标的名字下面那行)',
