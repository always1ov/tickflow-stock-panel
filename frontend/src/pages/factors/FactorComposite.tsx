    ...
    <div className="grid h-full min-h-0 grid-cols-1 gap-3 overflow-hidden xl:grid-cols-[20rem_minmax(0,1fr)]">
      <section className="flex min-h-0 flex-col gap-2 overflow-hidden rounded-card border border-border bg-surface/80 p-3">
        ...
        <div className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">
          {lib.isLoading && <div className="py-4 text-center text-micro text-muted">因子库加载中…</div>}
          {candidates.map(item => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between gap-2 rounded-btn border px-2 py-1.5 text-left text-xs transition-colors disabled:opacity-40 ${selected
                  ? SELECTED
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  {item.label}
                  <span className="ml-1 font-mono text-micro opacity-60">{item.id}</span>
                </span>
        ...
        <div className="border-t border-border pt-1.5 text-micro text-muted">
      ...
      <section className="flex min-h-0 flex-col gap-3 overflow-y-auto rounded-card border border-border bg-surface/80 p-3">
        <div className="flex items-center gap-2">
          ...
          <span className={TYPE.card}>组合因子构建器</span>
          <span className="text-micro text-muted">值 = Σ 权重 × 截面 z 分(成员)；注册后策略评分可选</span>
        </div>
            ...
            <div className="flex items-center gap-1.5 text-xs">
              ...
              {(['equal', 'manual'] as WeightMode[]).map(value => (
                <button
                  ...
                  className={buttonClass({ size: 'xs', selected: mode === value })}
                >
                  {value === 'equal' ? '均等' : '手动'}
                </button>
              ))}
              ...
              {memberList.map(([id, rawWeight]) => (
                <div key={id} className="flex items-center gap-2 rounded-btn border border-border bg-base/40 px-2.5 py-1.5">
                  <span className="min-w-0 flex-1 truncate text-xs text-foreground">
                    {labelOf(id)} <span className="font-mono text-micro text-muted">{id}</span>
                  </span>
                  {mode === 'manual' ? (
                    <>
                      <input
                      ...
                      />
                      <button type="button" onClick={() => toggleMember(id)} className="text-micro text-muted hover:text-danger">移除</button>
                    </>
                  ) : (
