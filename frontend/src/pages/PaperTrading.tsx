...
import {
  Bot, Eye, Loader2, Play, Plus, RotateCcw, Trash2, TrendingUp, X,
} from 'lucide-react'
import { api, type PaperOrder, type PaperTrader } from '@/lib/api'
...
export function PaperTrading() {
  ...
  const run = useMutation({
    mutationFn: (id: string) => api.paperTraderRun(id),
    onSuccess: res => {
      refresh()
      qc.invalidateQueries({ queryKey: ['paper-trader'] })
    ...
    <div className="flex min-h-full flex-col bg-base">
      ...
      <main className="min-h-0 flex-1 space-y-3 overflow-auto px-3 pb-4 pt-3 lg:px-4">
        <section className="rounded-card border border-border bg-surface px-3 py-2.5 text-[11px] leading-5 text-secondary">
          ...
          也互相看不见对方的持仓和理由 —— 上下文是按人组装的, 不是靠提示词叮嘱。
          撮合按 A 股规矩: 100 股一手、T+1 当天买的不能卖、按收盘价成交、算佣金印花税滑点。</span>
          <br />
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
            {traders.map(t => (
              <TraderCard key={t.id} t={t}
                busy={run.isPending && run.variables === t.id}
      ...
      {openId && <TraderDetail id={openId} onClose={() => setOpenId(null)} />}
    </div>
  )
}

function TraderCard({ t, busy, onRun, onOpen, onChanged }: {
  t: PaperTrader; busy: boolean
  onRun: () => void; onOpen: () => void; onChanged: () => void
}) {
  const reset = useMutation({
    mutationFn: () => api.paperTraderReset(t.id),
    onSuccess: () => { onChanged(); toast('已重置到起跑线', 'success') },
  ...
  return (
    <section className="flex flex-col rounded-card border border-border bg-surface">
      <header className="flex items-center gap-2 border-b border-border px-3 py-2">
        ...
        <button onClick={onOpen} className="min-w-0 flex-1 truncate text-left text-xs font-semibold text-foreground hover:text-accent">
          {t.name}
        </button>
        <span className={`shrink-0 font-mono text-sm font-bold ${pnlCls(t.return_pct)}`}>{pct(t.return_pct)}</span>
      </header>

      <div className="grid grid-cols-3 gap-2 px-3 py-2.5 text-center">
        <div>
          <div className="text-[9px] text-muted">总资产</div>
          <div className="mt-0.5 font-mono text-xs text-foreground">{money(t.nav)}</div>
        </div>
        <div>
          <div className="text-[9px] text-muted">持仓 / 现金</div>
          <div className="mt-0.5 font-mono text-xs text-foreground">{t.positions_count} 只 / {money(t.cash)}</div>
        </div>
        <div>
          <div className="text-[9px] text-muted">交易 / 天数</div>
          <div className="mt-0.5 font-mono text-xs text-foreground">{t.orders_count} / {t.days}</div>
        </div>
      </div>

      {t.last_note && (
        <div className="mx-3 mb-2 rounded border border-border/60 bg-elevated/30 px-2 py-1.5 text-[10px] leading-4 text-secondary">
          上次想法: {t.last_note}
        </div>
      )}
      {t.last_error && (
        <div className="mx-3 mb-2 rounded border border-danger/40 bg-danger/[0.06] px-2 py-1.5 text-[10px] leading-4 text-danger">
          上次没跑成: {t.last_error}
        </div>
      )}
...
function TraderDetail({ id, onClose }: { id: string; onClose: () => void }) {
  const [tab, setTab] = useState<'orders' | 'positions' | 'context'>('orders')
  const q = useQuery({ queryKey: ['paper-trader', id], queryFn: () => api.paperTrader(id) })
  const ctx = useQuery({
    queryKey: ['paper-trader-context', id],
    queryFn: () => api.paperTraderContext(id),
    enabled: tab === 'context',
  })
  const d = q.data
        ...
        {d && tab === 'orders' && (
          <div className="overflow-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {d.orders.map((o, i) => <OrderRow key={`${o.ts}-${i}`} o={o} />)}
                {d.orders.length === 0 && (
        ...
        {tab === 'context' && (
          <div className="min-h-0 flex-1 overflow-auto p-4">
            <p className="mb-2 text-[10px] leading-4 text-muted">
              这就是它下一次会拿到的<span className="text-secondary">全部</span>信息 —— 没有别的输入, 也没有联网。
