...
function RunStatus({ run, progress, error, reconnecting }: { run: MiningRun | null; progress?: { label?: string; phase?: string; percent?: number } | null; error?: string | null; reconnecting?: boolean }) {
  ...
  return (
    <div className={`flex min-w-0 items-center gap-2 border-b border-border px-3 py-2 ${error ? 'bg-danger/5' : 'bg-base/20'}`}>
      <Icon className={`h-3.5 w-3.5 shrink-0 ${reconnecting ? 'animate-spin text-warning' : active ? 'animate-spin text-accent' : success ? 'text-bear' : error ? 'text-danger' : 'text-muted'}`} />
    ...
    <div className="min-w-[760px]">
      ...
      {result.factors.map(row => (
        <div key={row.factor_name} className="grid grid-cols-[minmax(160px,1.5fr)_58px_repeat(7,minmax(72px,1fr))_minmax(130px,1.3fr)] border-b border-border/50 px-2 py-1.5 text-micro text-secondary last:border-b-0 hover:bg-elevated/40">
          ...
          <span className={row.selected ? 'text-bear' : 'truncate text-muted'} title={row.excluded_reason || undefined}>
        ...
        <div className="space-y-4 p-3">
          ...
          <section className="border-t border-border pt-3">
            ...
            <div className="max-h-40 space-y-1 overflow-y-auto">
              {(runsQuery.data?.items ?? []).map(run => <button key={run.run_id} type="button" onClick={() => attachRun(run)} className={`flex w-full items-center gap-2 rounded-btn px-2 py-1.5 text-left hover:bg-elevated ${task.runId === run.run_id ? 'bg-elevated font-medium' : ''}`}><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${SUCCESS.has(run.status) ? 'bg-bear' : ACTIVE.has(run.status) ? 'bg-accent' : 'bg-muted'}`} />{run.request?.auto && <span className="shrink-0 rounded-btn bg-accent/10 px-1 text-micro font-medium text-accent" title="自动挖掘（因子池由统计筛选生成）">自动</span>}<span className="min-w-0 flex-1 truncate font-mono text-micro text-secondary">{run.run_id}</span><span className="shrink-0 text-micro text-muted">{statusLabel(run.status)}</span></button>)}
