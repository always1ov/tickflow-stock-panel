/**
 * [R485] 庄现 —— 在副图上画一个狗头。**画法的唯一产地, 冻结。**
 *
 * 用户: 「现在想办法移植到量化macd里面, 当作辅助指标不在动了, 就算我以后微调量化macd
 * 也不动这些了」;「庄现用狗头 doge 这样更有含义」。
 * [R488] 用户: 「把狗头剥离量化macd, 放到趋势量化里面去」—— 现在画在趋势量化那张副图上,
 * 数据也改由 `/trend-quant` 带过来。
 *
 * 所以这里**不 import 任何一张副图的画法、不读它们的数值**: 狗头贴着所在副图的底边画在
 * 那一天的正下方, 位置只看「哪一天」—— 趋势量化 / 量化MACD 以后怎么调, 狗头的位置和
 * 出现的日子都不跟着变。算法在后端 `indicators/zhuang_xian.py`。
 *
