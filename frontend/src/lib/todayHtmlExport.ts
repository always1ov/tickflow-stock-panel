...
<div class="wrap">
  ...
  <div class="meta" style="margin:6px 0 0">把握分 = √(质地 × 时机),先过三道硬门槛才打分;两条竖线依次是质地与时机的得分。质地 = 趋势模板八条 / 相对强度 / 六态(以月计变化);时机 = 新鲜度 / 通道位置 / 量比 / 换手(逐日变化)。用几何平均是为了不让一边补另一边 —— 质地 95 时机 15 不该和两边都 55 打平。分数带 * 表示有因子缺数据,总分偏乐观。「走势」那一列是凭什么把这只挑出来:信号 + 六态 + 位置 + 量能 + 距关键点。位置与量能只说状态词 —— 要读懂 68% 和 1.82,得先知道多少算多。佐证(主线 / AI 信号 / 历史胜率 / 通道结论 / 量化波动通道的阶段与几何)在每行下面的小字里,一律不参与打分。</div>`
